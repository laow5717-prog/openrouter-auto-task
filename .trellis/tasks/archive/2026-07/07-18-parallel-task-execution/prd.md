...
- [x] AC1 **[已实跑]** `max_workers: 2` 确实并发。实跑 `/api/status` 观测到：
      ```
      parallel_mode: True   阶段2 轮次1 并发充值（3 个账号）
        W1: busy  轮次1 充值账号 qz7515q5al@web-library.net
        W2: busy  轮次1 充值账号 fl58bpop4a@web-library.net
      ```
      两个 worker 各自处理**不同**账号，符合"并发单位是账号"的设计。
...
- OQ2 并发是否会因同 IP 请求密度上升触发 Cloudflare 风控？
      **这是本次改造最大的未知数**——所有测试都打桩掉了浏览器，风控行为无从体现。
      2026-07-18 首次实跑（`max_workers=2`）未观察到明显风控迹象，但样本仅一轮，
      不足以定论。若触发，缓解顺序：降 `max_workers` 到 1 → 加大 `batch.interval_*`
      → 在 R6 基础上加全局节流。

## 实跑发现并修复的缺陷

- **支付卡争用被误判为卡池耗尽**（2026-07-18，commit `16ac210`）。首次实跑即暴露：
  一个 worker 正常付款时，另一个报「无可用支付卡（已试 0 张卡）」。根因是并发改造
  引入的 `PaymentCardRegistry` 把卡占到整个账号处理结束，卡池偏紧时饿死其它 worker，
  而 `cards_exhausted` 会让编排层 `done[email]='no_cards'` **永久放弃该账号**。
  修复：占用粒度改为单次支付尝试、被占用的卡不再永久跳过、区分"真耗尽"与"仅争用"。
  教训：这个缺陷 83 项自动化测试全部漏掉了——因为测试打桩了浏览器，而缺陷出在
  真实卡池的稀缺性与时序上。**打桩测试证明不了并发在真实资源竞争下的行为。**
