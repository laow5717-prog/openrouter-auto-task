...
**唯一未完成的是 AC10**：端到端跑通充值与订阅流水线，需要真实浏览器、AdsPower 客户端
在线、真实账号与会扣款的信用卡，只能由你来跑。清单见文末 Stage 4。
...
- [ ] **4.2** 跑通一个账号的 opencode 每日充值流水线，比对行为与改造前一致。**待你执行。**
- [ ] **4.3** 跑通一个账号的 opencode 订阅流水线。**待你执行。**
- [ ] **4.4** 首次上线时把 `config.yaml` 的 `adspower.reclaim_batch` 临时降到 1，观察一轮回收行为是否符合预期，确认无误后改回 3。**待你执行**——环境回收判据这次改动最大（三档判据全部重写），误删会丢登录态，值得先小步观察。
