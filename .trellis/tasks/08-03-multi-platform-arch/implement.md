...
分支 `feat/multi-platform-arch`，六个 commit：

| commit | 阶段 | 结果 |
|---|---|---|
| `dead94d` | Stage 0 前置清理 | driver.py 6157→1009 行；211 passed |
| `a52ca62` | Stage 1A 身份/平台分层 | V14；225 passed |
| `1b43e08` | Stage 1B 卡池平台化 | V15/V16；243 passed |
| `34cc90b` | Stage 2A 纯搬迁 + Stripe 抽层 | 243 passed，diff 全在 import 行 |
| `1d5d897` | Stage 2B/2C 协议与编排接入 | 255 passed；AC11 达成 |
| `530f713` | Stage 3 前端平台切换 | 262 passed |

测试从基线 210 passed + 1 skipped 增至 **262 passed，0 skipped，0 warning**。

生产库已迁移至 v16 并逐条核对：身份状态映射、3 条平台账号、2054 张卡的状态归属、
四张表的 platform 列全部正确，九张表行数与迁移前逐一相符。备份在
`data/openrouter_auto.db.bak-20260803-preplatform`（用 SQLite backup API 做的一致快照）。

**AC10 已于 2026-08-03 20:30 在生产环境实跑通过**（AdsPower 在线、真实账号、真实卡）：
充值链路试满 8 张卡命中 adapter 上限，7 张真实拒付判废、1 张 unknown 未被消耗；订阅链路
5 张全 unknown、一张未消耗，账号正常轮转、停止后资源干净释放。跨平台隔离在生产数据上
成立：两轮判废 9 张后 opencode 视角可选 432，另一平台视角仍是 971。详见 prd.md 的 AC10。

19 条验收标准全部勾选。唯一没被真实付款印证的是 `outcome='success'` 分支（当前卡池
里的卡全部拒付或超时），那一支只有单元测试覆盖。
...
- [x] **4.1** 在生产库上执行迁移（已备份）。已完成，数据逐条核对无误，见上方总览。
- [x] **4.2** 跑通一个账号的 opencode 每日充值流水线。已跑，8 张卡、结果与记账逐条核对无误。
- [x] **4.3** 跑通一个账号的 opencode 订阅流水线。已跑（`cunninghamh22` 完整走完 5 张卡转
  `registered_only`，并正常轮转到 `jot763`），手动停止后 worker / 代理 / AdsPower 环境全部释放。
- [ ] **4.4** 首次撞 AdsPower 配额时把 `config.yaml` 的 `adspower.reclaim_batch` 临时降到 1，
  观察一轮回收行为，确认无误后改回 3。**仍待执行**——两轮实跑都复用了已有环境、没触发回收，
  所以三档回收判据只有单元测试覆盖过。误删会丢登录态，第一次真正回收时值得小步观察。
- [x] **4.5** 全量 AC 走查，勾选 prd.md 的验收清单。19 条里 18 条已验证，AC10 待 4.2/4.3。

### 实跑观察到的（4.2 / 4.3 已完成，留作下次参考）

1. **不要在跑的时候改源码**。服务是 werkzeug dev 自动重载模式，改任何被监视的文件都会
   重启进程、把跑到一半的 worker 线程杀掉。第一次实跑就是这样被我自己打断的（只试了
   2 张卡就停），排查时一度以为是循环逻辑回归。中断后 AdsPower 环境会留在 Active，
   需要手动 `browser/stop`。
2. 启动门与流水线的账号数已经一致（都是 33），老毛病确认消失。
3. AdsPower 环境刚接管时首次导航可能失败一次（`⚠️ 导航 失败 @ about:blank`），
   `_safe_goto` 的重试会兜住，属正常。
4. 订阅结果判定 200 秒内经常判不出来（`unknown`），这是既有问题——`detect_subscribe_result`
   的 URL 回落判据代码注释里就写着「尚待真实付费标定」，不是本次改造引入的。

### 原本要留意的点（已验证）

1. **顶栏先选平台**。流水线启动接口现在要求 platform 必填，缺了直接 400。
2. **观察日志里的账号筛选数**。启动门与流水线现在用同一组终态常量，「启动时说有 N 个
   可充值账号、跑起来只有 M 个」这个老毛病应该消失了；若仍不一致说明还有漏网的过滤点。
3. **注意有没有账号被重复充值**。`_payable_now` 拆成两次查询后调整了读取顺序来收窄
   竞态窗口，单元测试连跑 6 轮无复现，但真实环境每步耗时长得多，值得盯一眼。
4. **确认 recharged 的账号不再跑 GitHub 注册**。这是状态分层顺带修掉的旧缺陷
   （原判据把 `status='recharged'` 当成未注册），在订阅流水线上最容易看出来。
