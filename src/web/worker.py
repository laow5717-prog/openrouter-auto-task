...
MIN_WORKERS = 1
# 有头 Chrome 每实例约 300-500MB。上限从 4 提到 10 是为了让单平台能跑满机器，
# 但这个数字**不是真正的天花板**——两道更硬的闸在它前面：
#   1. adspower.total_quota（11）是两平台共用的环境数硬上限，所以各平台的
#      并发度加起来不能超过它。10 只在「另一个平台基本不跑」时才拿得满。
#   2. 内存。10 个实例约 3-5GB，16GB 的机器跑满后会开始换页，吞吐反而下降，
#      崩溃的浏览器还会留下占着 profile 锁的孤儿进程。
# 换句话说：这里放开的是「代码允许多少」，实际能开多少要按配额和内存自己算。
MAX_WORKERS = 10
