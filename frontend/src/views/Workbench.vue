<template>
  ...
  <div class="info-banner">
    ...
    <span>
      <strong>每日任务：</strong>
      <strong>「开始充值」</strong>逐账号轮转充值（好卡优先、新卡垫后；一张卡能过款就一直用它连充，直到它失败才换下一张，达余额上限或试卡上限才换账号）；可充账号耗尽时，会自动拿列表里「已导入」的邮箱注册 GitHub，注册成功的账号下一轮直接进入登录充值——所以账号列表里只有新导入的邮箱也能直接开跑。
